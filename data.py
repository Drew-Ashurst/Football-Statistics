...
class Data:
    def __init__(self):
        ...
        chrome_options.add_argument("--no-sandbox")  # Disable the sandbox for compatibility
        chrome_options.add_argument("--disable-dev-shm-usage")  # Use /dev/shm instead of shared memory
        chrome_options.add_argument("--disable-gpu")  # Disable GPU (optional, for older environments)
    ...
    def get_player_team(self, row):
        try:
            return self.rosters.loc[
                self.rosters["Player"] == row["Player"], ["Team"]
            ].values[0][0]
        except:
            return None
    ...
    def get_indv_data(self):
        ...
        for url_key in keys.keys():
            ...
            while True:
                ...
                indv_df, state = self.get_next_page(
                    indv_df,
                    "/html/body/div[3]/main/section[3]/div/div/div/footer/div/div/a",
                )
                if not state:
                    break
            ...
            indv_df.to_csv(
                f"{config['individual_outputs']}/{url_key}_output.csv", index=False
            )
    ...
    def get_rosters(self):
        ...
        for team in self.teams:
            ...
            temp_df = pd.DataFrame(
                pd.read_html(f"https://www.nfl.com/teams/{name}/roster")[0]["Player"]
            )
            temp_df["Team"] = team
    ...
    def get_team_stats(self):
        offensive_urls = {
            i: f"https://www.nfl.com/stats/team-stats/offense/{i}/2024/reg/all"
            for i in config["offense_stat_list"]
        }
        defensive_urls = {
            i: f"https://www.nfl.com/stats/team-stats/defense/{i}/2024/reg/all"
            for i in config["defense_stat_list"]
        }
        st_urls = {
            i: f"https://www.nfl.com/stats/team-stats/special-teams/{i}/2024/reg/all"
            for i in config["special_teams_stat_list"]
        }
        dicts = [
            (offensive_urls, "offense"),
            (defensive_urls, "defense"),
            (st_urls, "special_teams"),
        ]
        for dict in dicts:
            side = dict[1]
            for url_key in dict[0].keys():
                ...
                while True:
                    team_df, state = self.get_next_page(
                        team_df,
                        "/html/body/div[3]/main/section[3]/div/div/div/footer/div/div/a",
                    )
                    if not state:
                ...
                team_df.to_csv(
                    f"{config['team_outputs']}/{side}/{side}_{url_key}_output.csv",
                    index=False,
                )
    ...
    def get_game_outcomes(self, current_week):
        ...
        for week in range(1, current_week + 1):
            ...
            team_list = soup.find_all(
                ...
                {
                    "class": [
                        "css-text-146c3p1 r-color-1khnkhu r-fontFamily-1fdbu1n r-fontSize-ubezar"
                    ]
                },
            )
            scores = soup.find_all(
                ...
                {
                    "class": [
                        "css-text-146c3p1 r-fontFamily-1ujtvat r-fontSize-ubezar r-marginLeft-1jkjb r-textAlign-q4m81j r-width-lrsllp r-color-zyhucb",
                        "css-text-146c3p1 r-fontFamily-1ujtvat r-fontSize-ubezar r-marginLeft-1jkjb r-textAlign-q4m81j r-width-lrsllp r-color-1khnkhu",
                    ]
                },
            )
            ...
            matchups = [
                [team_list[i], team_list[i + 1]] for i in range(0, len(team_list), 2)
            ]
            matchup_scores = [
                [scores[i], scores[i + 1]] for i in range(0, len(scores), 2)
            ]
                ...
                data.append(
                    {
                        ...
                        "Outcome": (
                            "Win"
                            if matchup_scores[i][0] > matchup_scores[i][1]
                            else "Loss"
                        ),
